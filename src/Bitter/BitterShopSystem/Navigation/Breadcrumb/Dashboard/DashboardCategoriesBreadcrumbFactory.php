...
class DashboardCategoriesBreadcrumbFactory implements ApplicationAwareInterface
{
    ...
    public function getBreadcrumb(Page $dashboardPage, $mixed = null): BreadcrumbInterface
    {
        ...
        if ($mixed instanceof Category) {
            /** @noinspection PhpUnhandledExceptionInspection */
            $item = new Item(
                $this->app->make('url')->to(
                    '/dashboard/bitter_shop_system/products/categories', 'update', $mixed->getId()
                ),
                $mixed->getId()
            );
