...
class DashboardOrdersBreadcrumbFactory implements ApplicationAwareInterface
{
    ...
    public function getBreadcrumb(Page $dashboardPage, $mixed = null): BreadcrumbInterface
    {
        ...
        if ($mixed instanceof Order) {
            /** @noinspection PhpUnhandledExceptionInspection */
            $item = new Item(
                $this->app->make('url')->to(
                    '/dashboard/bitter_shop_system/orders', 'update', $mixed->getId()
                ),
                $mixed->getId()
            );
