...
class DashboardTaxRatesBreadcrumbFactory implements ApplicationAwareInterface
{
    ...
    public function getBreadcrumb(Page $dashboardPage, $mixed = null): BreadcrumbInterface
    {
        ...
        if ($mixed instanceof TaxRate) {
            /** @noinspection PhpUnhandledExceptionInspection */
            $item = new Item(
                $this->app->make('url')->to(
                    '/dashboard/bitter_shop_system/tax_rates', 'update', $mixed->getId()
                ),
                $mixed->getId()
            );
