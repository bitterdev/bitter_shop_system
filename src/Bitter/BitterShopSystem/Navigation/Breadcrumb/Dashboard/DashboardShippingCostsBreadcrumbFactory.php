...
class DashboardShippingCostsBreadcrumbFactory implements ApplicationAwareInterface
{
    ...
    public function getBreadcrumb(Page $dashboardPage, $mixed = null): BreadcrumbInterface
    {
        ...
        if ($mixed instanceof ShippingCost) {
            /** @noinspection PhpUnhandledExceptionInspection */
            $item = new Item(
                $this->app->make('url')->to(
                    '/dashboard/bitter_shop_system/shipping_costs', 'update', $mixed->getId()
                ),
                $mixed->getId()
            );
