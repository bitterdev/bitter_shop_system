...
class OrderListPagerManager extends AbstractPagerManager
{
    /** 
     * @param Order $order
     * @return int 
     */
    public function getCursorStartValue($order)
    {
        return $order->getId();
    }
    ...
    public function getCursorObject($cursor)
    {
        ...
        return $entityManager->getRepository(Order::class)->findOneBy(["id" => $cursor]);
    }
    ...
    public function sortListByCursor(PagerProviderInterface $itemList, $direction)
    {
        ...
        $itemList->getQueryObject()->addOrderBy('t4.id', $direction);
    }
}
